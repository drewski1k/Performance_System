...
# ── Static routes (must come before {metric_id} routes) ─────────────────────
...
@router.post("/definitions/validate-formula")
def validate_formula_standalone(body: dict, db: Session = Depends(get_db)):
    """Validate a formula string against available metric keys (no metric_id required)."""
...
# ── Parameterized routes ({metric_id}) ──────────────────────────────────────
