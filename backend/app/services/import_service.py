...
_COMBINED_COLUMN_MAP = {
    ...
    "Phone Calls Accepted": ("contact_accepted_voice", "direct"),
    "Chats Accepted": ("contact_accepted_chat", "direct"),
    "Emails Accepted": ("contact_accepted_email", "direct"),
...
def process_glance_report(df: pd.DataFrame) -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        metrics: dict[str, float] = {
            ...
            "contact_accepted_voice": voice_accepted,
            "contact_accepted_chat": chat_accepted,
            "contact_accepted_email": email_accepted,
...
def validate_combined_import(records: list[dict]) -> dict:
    ...
    for r in records[:20]:
        ...
        preview.append({
            ...
            "voice_contacts": int(m.get("contact_accepted_voice", 0)),
            "chat_contacts": int(m.get("contact_accepted_chat", 0)),
            "email_contacts": int(m.get("contact_accepted_email", 0)),
